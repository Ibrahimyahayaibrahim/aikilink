...
/* ---------- interactive rating input (self-contained) ---------- */
...
function RatingInput({ value, onChange }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex items-center gap-1" onMouseLeave={() => setHover(0)}>
        {[1, 2, 3, 4, 5].map((i) => (
          <button
            ...
            onBlur={() => setHover(0)}
            onClick={() => onChange(i)}
            className="btn-press rounded-full p-1 transition-transform duration-150 hover:scale-125 active:scale-90"
          >
            <Star
              className={`h-9 w-9 transition-all duration-200 md:h-10 md:w-10 ${
                i <= shown ? "fill-amber-400 text-amber-400 drop-shadow-sm" : "text-line hover:text-amber-300"
              }`}
              strokeWidth={1.5}
            />
          </button>
      ...
      <span className={`h-5 text-sm font-bold transition-colors ${shown ? "text-amber-600" : "text-mist/60"}`}>
...
/* small read-only stars for provider averages */
...
export default function JobDetailHomeowner() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      ...
      <section className="anim-up overflow-hidden rounded-2xl border border-line bg-card shadow-sm" style={{ animationDelay: "0.06s" }}>
        <div className="dot-grid relative bg-pine px-5 py-4 md:px-6">
          ...
          <div className="relative flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center rounded-full bg-cream/15 px-2.5 py-1 text-[11px] font-bold text-cream">
          ...
          {interested.length === 0 ? (
            ...
            <div className="mt-5 space-y-3">
              {interested.map((p, i) => (
                <div
                  key={p.providerId}
                  className="anim-up group flex flex-col gap-3 rounded-2xl border border-line bg-cream/50 p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-pine/25 hover:shadow-lg sm:flex-row sm:items-center sm:justify-between"
                ...
                >
                  ...
                  <button
                    disabled={busy}
                    onClick={() => handleClaim(p.providerId)}
                    className="btn-press inline-flex shrink-0 items-center justify-center gap-1.5 rounded-2xl bg-amber-500 px-5 py-2.5 text-sm font-bold text-pine-900 shadow-sm hover:bg-amber-400 hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-60"
                  >
      ...
      {/* ---------- CLAIMED / IN PROGRESS: artisan spotlight ---------- */}
        ...
        <section className="anim-up rounded-2xl border border-line bg-card p-5 shadow-sm md:p-6" style={{ animationDelay: "0.18s" }}>
          ...
          <div className="mt-5 flex flex-col gap-2.5 sm:flex-row">
            <button
              ...
              className="btn-press inline-flex flex-1 items-center justify-center gap-2 rounded-2xl bg-amber-500 py-3.5 text-sm font-bold text-pine-900 shadow-md hover:bg-amber-400 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <button
              ...
              className="btn-press inline-flex items-center justify-center gap-2 rounded-2xl border border-rose-300 px-5 py-3.5 text-sm font-bold text-rose-600 transition-colors hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60"
            >
      ...
      {status === "Completed" && !ratingSubmitted && (
        <form onSubmit={handleSubmitRating} className="anim-up rounded-2xl border border-line bg-card p-6 text-center shadow-sm md:p-8" style={{ animationDelay: "0.18s" }}>
          ...
          <p className="mx-auto mt-1 max-w-sm text-[13px] font-medium text-mist">
            Your review helps other homeowners hire with confidence.
          </p>
          <div className="mt-5">
          ...
          <textarea
            ...
            rows={3}
            className="mt-5 w-full resize-none rounded-2xl border border-line bg-cream px-4 py-3 text-sm text-ink outline-none transition-all duration-200 placeholder:text-mist/60 focus:border-amber-500 focus:bg-card focus:ring-2 focus:ring-amber-500/25"
          />
          <button
            type="submit"
            disabled={busy || ratingScore === 0}
            className="btn-press mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-amber-500 py-3.5 text-sm font-bold text-pine-900 shadow-md hover:bg-amber-400 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
          >
      ...
      {status === "Completed" && ratingSubmitted && (
        <div className="anim-scale rounded-2xl border border-emerald-200 bg-emerald-50 p-6 text-center" role="status">
          <span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-emerald-500 text-cream">
